document.addEventListener("DOMContentLoaded", () => {
    ...
    const generateJSON = () => {
        ...
        decksContainer.querySelectorAll(".deck").forEach((deckEl) => {
            const magicItemName =
                deckEl.querySelector(".magic-item-name").value;
        ...
        // Wrap JSON in single quotes and backspace apostrophes 
        jsonOutput.value = `'${JSON.stringify(magicItems).replace(
            /['‘’‚‛]/g,
            "\\'"
        )}'`;
    };
    ...
    `;
        cardDiv
            .querySelector(".remove-deck-card")
